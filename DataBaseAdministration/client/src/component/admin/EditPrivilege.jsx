...
function EditPrivileges() {
  ...
  const getPrivilegeUrl = `http://localhost:4000/api/privileges/${id}`;
  const updatePrivilegeUrl = `http://localhost:4000/api/privileges/${id}`;
 ...
 useEffect(() => {
  async function fetchData() {
    setLoading(true);
    try {
      const [privilegeRes, tablesRes, privilegesRes] = await Promise.all([
        axios.get(getPrivilegeUrl),
      ...
      if (privilegeRes.status === 200) {
        const { privilege_name, description, tables } = privilegeRes.data.data;
        setPrivilegeName(privilege_name);
        setDescription(description);
        setTables(tables || []);
      }
      const tablesData = tablesRes.data.data && Array.isArray(tablesRes.data.data) ? tablesRes.data.data.filter(t => t) : [];
      setAvailableTables(tablesData);
      setExistingPrivileges(privilegesRes.data.data || []);
      setError(null);
    } catch (error) {
      const errorMsg = error.response?.data?.error || "Error fetching privilege or tables";
      setError(errorMsg);
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const res = await axios.put(updatePrivilegeUrl, payload);
      toast.success("Privilege updated successfully");
      setTimeout(() => navigate("/privileges"), 1000);
    } catch (error) {
      const errorMsg = error.response?.data?.error || "Error updating privilege";
      setError(errorMsg);
      toast.error(errorMsg);
      console.error("Error:", error);
    } finally {
