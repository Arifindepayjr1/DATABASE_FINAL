...
function UpdateRoles() {
  ...
  const getRoleUrl = `http://localhost:4000/api/roles/${id}`;
  const updateRoleUrl = `http://localhost:4000/api/roles/${id}`;
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [roleRes, tablesRes, privilegesRes] = await Promise.all([
        ...
        if (roleRes.status === 200) {
          const { role_name, description, privileges } = roleRes.data.data;
          setRoleName(role_name || "");
          setDescription(description || "");
          setSelectedPrivileges(
            privileges.map((p) => String(p.privilege_id)).filter(Boolean)
          );
        }
        ...
        const errorMsg = error.response?.data?.error || "Error fetching role or tables";
        setError(errorMsg);
  ...
  async function handleSubmit(e) {
    ...
    try {
      const res = await axios.put(updateRoleUrl, payload);
      if (res.status === 200) {
      ...
      const errorMsg = error.response?.data?.error || "Error updating role";
      setError(errorMsg);
      toast.error(errorMsg);
